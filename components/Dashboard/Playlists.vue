    ...
    >
      <template #[`item.actions`]="{ item }">
        <v-btn
          ...
          >List Playlist</v-btn
        >
        <v-btn v-else color="primary" @click="openPricingDialog(item)"
          >Edit (${{ item.price }})</v-btn
        >
        <v-btn
          v-if="item.curator_playlist"
          color="primary"
          @click="promotePlaylist(item)"
          >Promote</v-btn
        >
        <v-btn
          v-if="item.curator_playlist"
        ...
        <v-card-text>
          <v-text-field
            ...
            :rules="[
              (v) =>
                v <= $auth.user.price ||
                `Price cannot be above $${$auth.user.price}`,
            ]"
...
export default {
  ...
  methods: {
    ...
    async setPrice() {
      if (this.price > this.$auth.user.price) {
        return
      }
      try {
